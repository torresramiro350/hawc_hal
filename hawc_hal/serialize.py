...
class Serialization(object):
    def __init__(self, filename: str, mode: str = "r", compress=True):
    ...
    def __enter__(self):
        if self._compress:
            self._store = HDFStore(
                self._filename, complib="blosc:lz4", complevel=9, mode=self._mode
            )
    ...
    def store_pandas_object(self, path: str, obj, **metadata):
