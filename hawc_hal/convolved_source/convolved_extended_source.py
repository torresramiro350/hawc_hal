...
class ConvolvedExtendedSource3D(ConvolvedExtendedSource):
    ...
    def _parameter_change_callback(self, this_parameter):
        ...
        # NOTE: we do not recompute it here because if more than one parameter changes
        # at the time (like for example during sampling) we do not want to recompute
        # the function multiple time before we get to the convolution stage.
        # Therefore we will compute the function in the get_source_map method
        ...
        with use_astromodels_memoization(False):
            ...
            for dec_bin1, dec_bin2 in zip(
                ...
                this_model_image[idx] = (
                    w1
                    * np.sum(scale * this_response_bin1.sim_signal_events_per_bin, axis=1)
                    + w2
                    * np.sum(scale * this_response_bin2.sim_signal_events_per_bin, axis=1)
                ) * 1e9
