...
class ConvolvedPointSource(object):
    ...
    def _update_dec_bins(self, dec_src):
        if abs(dec_src - self._last_processed_position[1]) > 0.1:
            ...
            self._response_energy_bins = self._response.get_response_dec_bin(
                dec_src, interpolate=True
            )
    ...
    def get_source_map(
        self, response_bin_id, tag=None, integrate=False, psf_integration_method="fast"
    ):
        ...
        ra_src, dec_src = (
            self._source.position.ra.value,
            self._source.position.dec.value,
        )
