from hawc_hal.healpix_handling.flat_sky_to_healpix import (
    _parse_coord_system,
    _convert_world_coordinates,
    ORDER,
)
...
def image_to_healpix(
    ...
    healpix_data_ = map_coordinates(
        data, [xinds, yinds], order=order, mode="constant", cval=fill_value
    )
    ...
    if not full:
        ...
        if pixels_to_be_zeroed is not None:
            healpix_data[pixels_to_be_zeroed] = np.where(
                np.isnan(healpix_data[pixels_to_be_zeroed]),
                0.0,
