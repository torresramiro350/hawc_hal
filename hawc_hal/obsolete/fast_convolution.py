...
class FastConvolution(object):
    ...
    def __call__(self, image):
        assert alltrue(image.shape == self._expected_shape), (
            "Shape of image to be convolved is not correct. Re-run setup."
        )

        ret = irfftn(rfftn(image, self._fshape) * self._psf_fft, self._fshape)[
            self._fslice
        ].copy()
...
def brute_force_convolution(image, kernel, output):
    ...
    for i in range(half_size_minus_one, h - half_size_minus_one):
        for j in range(half_size_minus_one, w - half_size_minus_one):
            output[j, i] = np.sum(
                image[
                    j - half_size_minus_one : j + half_size_minus_one + 1,
